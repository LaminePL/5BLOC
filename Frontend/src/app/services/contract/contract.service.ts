...
export class ContractService {
  ...
  buyTicket(departure,arrival,name, price ){
    return new Promise((resolve, reject) => {
      ...
      paymentContract.deployed().then((instance) => {
        return instance.buyTickets( departure, arrival, name,price,{ from: this.accounts[0], value: this.web3.utils.toWei("2", 'ether') }
          );
      }).then((status) => {
        if (status) {
  ...
  trasnferEther(originAccount, destinyAccount, amount) {
    ...
    return new Promise((resolve, reject) => {
      ...
      paymentContract.deployed().then((instance) => {
        ...
        return instance.nuevaTransaccion( destinyAccount,{ from: originAccount[0], value: this.web3.utils.toWei(finalAmount, 'ether') }
          );
      }).then((status) => {
        if (status) {
          return resolve({status: true});
